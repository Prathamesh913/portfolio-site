...
type Reading = {
  ...
  url?: string;
  /** Source-confirmed Hardcover edition cover (assets.hardcover.app). */
  cover?: string;
} | null;
...
async function getWatching(): Promise<{ data: Watching; live: boolean; debug: TraktDebug }> {
  ...
  try {
    ...
    if (!histRes.ok) {
      // Rejected Bearer token (expired/revoked/rotated) — flag re-auth even
      // when the refresh step itself didn't report invalid_grant.
